...
# tune_RF = expand.grid(mtry = c(1, 2, 5, 10, 15, 20, 25, 30)) #Number of variables randomly sampled as candidates at each split.
#                       #ntree (not tunned) - set default as 500
...
for (variant_id in list_of_variants) {
  ...
  save(list = paste("RF_fit_", toString(variant_id), sep = ""),
       file = paste("D:\\Doktorat\\wyniki\\", area, "\\wyniki_kroswalidacja\\",scale, "\\Regresja\\Random_Forest\\", image_name, "\\wyniki_kroswalidacja_", toString(variant_id),".RData", sep = ""))
  
  write.csv(RF_fit$pred, 
             file = paste("D:\\Doktorat\\wyniki\\", area, "\\wyniki_kroswalidacja\\",scale, "\\Regresja\\Random_Forest\\", image_name, "\\pred_", toString(variant_id),".csv", sep = ""), 
             row.names = FALSE)

  write.csv(RF_fit$results, 
             file = paste("D:\\Doktorat\\wyniki\\", area, "\\wyniki_kroswalidacja\\",scale, "\\Regresja\\Random_Forest\\", image_name, "\\results_", toString(variant_id),".csv", sep = ""),
             row.names = FALSE)
